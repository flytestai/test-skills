...
USER_PADDLEX_CACHE = Path.home() / ".paddlex" / "official_models"
...
def prepare_runtime_and_models(target_dir: Path) -> None:
    ...
    ensure_model_cache(target_dir)
    model_files = list((target_dir / "scripts" / ".paddlex-cache").rglob("*"))
    real_files = [path for path in model_files if path.is_file()]
    if not real_files:
        raise RuntimeError("离线模型预热失败：scripts/.paddlex-cache 下未生成任何模型文件")


def ensure_model_cache(target_dir: Path) -> None:
    target_cache_root = target_dir / "scripts" / ".paddlex-cache"
    target_models_dir = target_cache_root / "official_models"
    target_cache_root.mkdir(parents=True, exist_ok=True)

    existing_files = [path for path in target_cache_root.rglob("*") if path.is_file()]
    if existing_files:
        return

    if USER_PADDLEX_CACHE.exists():
        print(f"检测到本机已有 PaddleX 模型缓存，正在复制到离线包：{USER_PADDLEX_CACHE}")
        shutil.copytree(USER_PADDLEX_CACHE, target_models_dir, dirs_exist_ok=True)
